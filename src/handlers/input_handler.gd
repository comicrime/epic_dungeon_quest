...
func _ready() -> void:
	...
	MsgBus.player_died.connect(transition_to.bind(InputHandlers.GAME_OVER))
	
func get_action(player: Entity) -> Action:
	return current_input_handler.get_action(player)

func transition_to(input_handler: InputHandlers) -> void:
	if self.current_input_handler != null: 
		current_input_handler.exit()

	current_input_handler = input_handler_nodes[input_handler] as BaseInputHandler
